...
| `dr_render.py` | Item 7's renderer: the Decision Record page from persisted graphs and nothing else. G1-pure (stdlib + `mindsos_core` only); every gap raises (`RendererGapError`); render-time G6 scans its own page. ⚠ **The page also names the RULE a verdict was measured against** and the edition that stated it (step 2) — a second structural marker, `measured_against`, resolved through the SAME pairing the deciding fact uses. **Nothing is rendered where nothing was applied:** a vehicle exposure routed on coverage alone and a refusal that never reached the comparison both carry no rule line, because a page citing an authority it did not consult is the same defect as a renderer composing an outcome word. |
...
| `dr_routing.py` | The routing content (plan §2.5): exposures → desks on the Guidewire-sourced model. Beat 1 (one claim, two desks) and beat 2 (a refusal beside an answer) — a MAP over the exposures of one claim plus the reducer that assigns the claim. ⚠ **The rule is STORED, DATED POLICY, not a Python conditional** (step 2): `_route` compares a claimant's stated weeks off work against a threshold read by `policy_lookup_v0` from a dated edition, exactly as beat 4 reads the dwelling limit. **Two editions ship** — eight weeks in 2023, four from 2024 — so the same exposure reaches different desks under different editions and the page names which one sent it there. ⚠ **ONE threshold, not the disjunction** (plan §0.5 item 4): the chosen fixture satisfies both disjuncts, `DETERMINED_BY` holds one DataState, and the page would have stated a reason that was not counterfactually the reason. `routing_policy_file(as_of)` prints the edition as the file a room is shown, out of stored words only. |
...
**Guard total: 114** (render 37, routing 24, transport 14, assessment 13, screen 10, run 5, beat 5, no-model 3, dump 3) — 106 before step 2, 92 before step 1, 68 before ship B, 56 before the deciding-fact ship.
