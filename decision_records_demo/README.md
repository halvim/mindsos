...
| `test_dr_render_guards.py` | 36 guard tests (G1/G2/G5/G6, the §30 rulings, the correlation bijection, the stated-absence rule, the policy source line and its contract pin, beat 3's refusal, the deciding fact on the leaf road, and ship B's refusing-leaf verdict line — a refusing leaf NAMES the decision it could not make, the member road does NOT gain that line because its fold already states the consequence, and two unconsumed refusal carriers raise). Plain python or pytest; no FalkorDB. |
