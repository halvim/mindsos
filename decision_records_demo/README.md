...
| `dr_render_pages.py` | The §12 command for the renderer: ten cases → a real store → each page rendered FROM the store, then an end-state re-verify of every page from the store alone. `--from-root <capacity_root_ref>` renders from the store ALONE (no live KL) — the reconstructibility proof, a **Gate-7 predecessor**. `--screens <dir>` writes the Screen-A HTML per case. |
...
**Guard total: 68** (render 33, routing 11, screen 10, run 5, no-model 3, beat 3, dump 3) — 56 before the deciding-fact ship.
