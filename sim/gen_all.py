...
def checklist(name, arm, kind="box"):
    ...
    box = ("box1" if arm == 1 else "box2") if kind == "box" else \
          ("tube1" if kind == "tube_pick" else "sheet1")
    oi = bs.index(box)
    pf = f"a{arm}_"
    ...
    if kind in ("sheet_pick", "tube_pick"):       # reverse runs: final rest is ON THE BELT
        floor = G.BELT_Z; box_hz = 0.010 if kind == "sheet_pick" else 0.035
    else:
        floor = (G.ROW_Z0 + row * G.ROW_DZ) - 0.5 * G.ROW_DZ + 0.008
        box_hz = 0.05 if kind == "box" else 0.044  # sheet stands on its (rotated-up) y half
...
REF = {}     # was {(1, 0, 2): (0, 1)}: neighbour-rotate detour — replaced by a direct carry
             # (user asked for the smaller path); restore the entry to get the dogleg back
