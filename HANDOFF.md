...
> 4446/0. They redefine what a **plan**, a **milestone** and a **pipeline** are, rule that
> **concepts are the source of truth and code matches concepts**, and re-sequence core work
> out of the WSD slots. **Nothing is built yet.** Start at
> `confirmation_docs/CORE_RECONCILIATION_PLAN.md` (the CORE-C build plan, shim register,
> rename inventory), then `CORE_VERIFIED_FINDINGS.md`. Core work no longer uses phase
> numbers — `feat/*` branches + `<name>-confirmed` tags. The narrative banner below is
> from 2026-06-09 and predates all of this.
>
> **2026-07-31, later the same day — first CORE-C code ship.** `4fd8baa`, tag
> `finder-cycle-guards-confirmed`, gate 4450/0. Two `ConjunctionFinder` defects that were
> live on the CLI `execute` verb: **D-B** (phase 2 discarded phase 1's cycle stack) and
> **D-E** (a capacity under construction was invisible to both guards, so `find` could
> return a `Pipeline` naming the same capacity twice and report success). ADR-0071 §am-3.
> **This is a patch, not the design** — the agreed successor turns the finder into four
> dispatched capacities computed bottom-up as a fixpoint. Spec, with eight already-rejected
> alternatives, is `confirmation_docs/CORE_CR_FINDER_AS_CAPACITIES.md`; the 20,000-graph
> evidence model is `confirmation_docs/finder_variants_model.py`. Still open in that lane:
> the `find_verdict` type and the divergence sweep.
>
>
> **2026-08-01 — CORE-C1R4, the ADR contradiction sweep.** `2c56246` (PR #104), docs-only,
> no tag. The sweep's converged answer is that **no abstraction level above `capacity` has a
> graph representation at all**: `IntergraphHyperEdge`/`compositional` has zero users outside
> `mindsos_instances`, `chain_artifacts.py` has zero `add_edge`, `Pipeline.edges` is read only
> by `to_dict()`, and the plan ships as a dict inside an opaque DataState value. CORE-C2 is
> therefore building the first representation, not migrating one. The decisions are in
> **ADR-0205 §amendment-1** (Proposed; flips at C2R1) — composition primitive selected **by
> arity**, subsumption is not composition, compositional is **terminal**, plus the criterion's
> missing scope clause and two factual corrections to ADR-0205 itself. **§amendment-1.6 blocks
> C2R2** and must be decided before the milestone graph is built. Coverage ledger:
> `confirmation_docs/CORE_ADR_CONTRADICTION_SWEEP.md`. **Owed:** the per-ADR amendments (21
> `contradicts` + 9 `contradicts-if-built`) — C1R4 part 2.
>
> **2026-08-01 — both 2026-07 architecture guards were repaired; one had never run.**
> `tools/check_adr_status_consistency.py` (PR #106, `1532e6c`, gate 4456/0) claimed to assert an
> ADR's status agrees across four places and **only ever checked two** — its table-header
> detection never matched the README's header, so `check_index` was a no-op on the one file it
> exists to police. The index had stopped at **ADR-0137**: 76 of 204 files unlisted (including
> ADR-0205/0206) and 18 rows disagreeing with their file, 8 of them showing a **Superseded**
> decision as **Accepted**. `tests/architecture/test_no_subsystem_ownership.py` (PR #108,
> `60c0fd6`, gate 4459/0) did run, but missed the phrasings its own rule names — `deferred to
> WSD`, `routed to WSD`, `owned by WSD`, `WSD ships …` — and its ALLOWLIST exempted nothing while
> a test stood watch over it. **New hard rule: adding an ADR file requires adding its README row,
> or the gate fails** (RULES §9). In-file amendments need no row but must label their status
> `**Amendment status:**`, never `**Status:**`. Standing lesson, now in RULES §9: **a green guard
> that cannot fail is worse than no guard — test the failure behaviour, not only the passing
> state.**
