...
> **Last updated:** 2026-06-07 (Phase 42 **SHIPPED**; Rail B — L3 X3 — completes Rail B X1→X2→X3. ADR-0156 bipartite topology + ADR-0159 capacity registration contract v2 + Phase 27 dont-know audit (L3-57) + Model C remediation. `register_capacity` emits explicit `PRODUCES`/`CONSUMES` IntergraphEdges (uppercase rel-type); `discovery.py`/TYPE_COMPAT/`SuccessorHop`/`DiscoveryFailedError`/`rediscover` retired; edge-sourced view walks + bipartite `find_pipeline`; +6 `_CapacityBase` fields + typed `CapacityContext` + 4 Protocols + 5 verdicts (`context.py`); `mindsos_instances` IntergraphEdge/HyperEdge instances (catalog 8→10); detector not migrator (PB-7); `FAMILY_RULES` reconciliation (PB-8 Opt 3); `__all__` 112→117. **Deferred to Phase 46 (PB-23):** capacity-body `context["kl"]`→`context.kl` + `invoke`→CapacityContext plumbing + instance `materialise`. 3669 passed cumulative gate; tag `phase-42-confirmed` at confirm-artifacts `e0a1453` (ship squash `39a312c`); no version bump (slot 42 ≤ 44). See §3.1.17 for ship closure. Prior: Phase 41 SHIPPED 2026-06-05, §3.1.16; Phase 40 §3.1.15; Phase 44 §3.1.14.)
