...
**A transport failure is a refusal, and it is flagged as ours.**
``model_unreachable`` sets ``environment_fault`` on the record. *"The
document does not say"* is a finding about the customer's case; *"our
reading service was down"* is an outage, and it must never pad a customer's
refusal list.
    ...
    *,
    ...
    def _impl(**kwargs: Any) -> Dict[str, Any]:
        ...
        if not isinstance(source_text, str) or not source_text.strip():
            return _refuse(REFUSAL_MALFORMED_RESPONSE, "no source text was supplied to read")
        ...
        try:
            ...
            # A transport failure is OUR fault, not a fact about the case.
            # It becomes a graph-resident refusal — otherwise a run that
            # dies on venue wifi leaves nothing to render — carrying
            # ``environment_fault`` so it never pads a customer's refusal
            # list. Budget and replay-miss faults deliberately still raise
            # (a run-level policy limit and a fault in our recorded set).
            name = type(exc).__name__
            if name in ("LLMCallBudgetExceeded", "RecordedResponseMiss"):
                raise
            return _refuse(
                REFUSAL_MODEL_UNREACHABLE,
                f"the reading service could not be reached ({name}: {exc})",
            )

        if not isinstance(response, Mapping):
            return _refuse(
                REFUSAL_MALFORMED_RESPONSE,
                f"the reading service returned {type(response).__name__}, expected a mapping",
            )
