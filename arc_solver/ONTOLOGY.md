...
| 20 | Capacity / DataState / pipeline grounding (crux) | **A capacity IS a DataState transition** (`consumes → produces`). **DataState is the universal unit** — anything in L2/L5 *is* a DataState; a cap declares only in_ds/out_ds **types**, and **L4 binds each to a source (input/L2/L5) + dest (output/L2/L5)**. **No "L4 capacities":** L4 is the *substrate* (Executor/Dispatcher/MM) that iterates/dispatches/routes; orchestration *decisions* are L3 caps (`planning.*`/`orchestration.*`/`phase1.*`). **Aggregates** (profile, correspondence, rules…) are normal L3 DataStates **stored in L5** by decision caps + L4, then sourced — no ∅-producer. **Groups (`*`, C4):** a group DataState (list/set) is a *distinct type* from its member; L4 iterates it (fan-out needs no finder cardinality). **`find_pipeline` = discovery of UNKNOWN compositions**; **known** pipelines (perceive) are recorded in **L2 `promoted-pipelines`**. Four core gaps filed (`CORE_REQUESTS.md`): **C1** same-type operand arity · **C3** truthful invoke contract · **C4** group/member attribute · **C5** known-pipeline writer+lookup. Full inventory: `ATOM_TABLE.md`. *(v0.10; 2026-07-06.)* |

---

## 5. Changelog

- **v0.10** (2026-07-06) — **Capacity/DataState/pipeline grounding (§4 #20).** Settled the crux from
  the atom-grounding chat: capacity = DataState transition; DataState = universal unit with L4-bound
  source/dest (input/L2/L5); no "L4 capacities" (L4 = substrate; orchestration decisions are L3 caps
  in `planning.*`/`orchestration.*`); aggregates are L3 DataStates stored in L5 (no ∅-producer);
  groups (`*`, C4) are distinct types L4 iterates; `find_pipeline` = unknown-composition discovery,
  known pipelines → L2 `promoted-pipelines`. Four core gaps filed (`CORE_REQUESTS.md`: C1 arity /
  C3 truthful contract / C4 group-member / C5 known-pipeline). Working inventory: `ATOM_TABLE.md`.
  No code (docs-only); shipped shim unchanged (reconciliation = a later gated chat).
