...
L3 partitions capacities into **12 functional categories** (ADR-0065). Every `Capacity` / `Monitor` / `Adapter` MUST declare exactly one home category; the IRI form `capacity:<category>:<name>` reflects it.
...
| Trace | `CATEGORY_TRACE` | Reading / writing problem-trace records |
...
The enumeration is canonical; `FUNCTIONAL_CATEGORIES` is the frozenset of all 12. Extending requires an ADR-0065 amendment.
