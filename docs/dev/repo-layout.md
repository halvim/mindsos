...
├── mindsos_knowledge/    # Domain Layer 2 — IRIs + REF_TYPES + 17 role-schema builders (closed set, ADR-0150).
│   ├── exceptions.py     # KnowledgeError, RefFormatError, UnknownRoleError.
│   ├── identifiers.py    # ~24 IRI builders + parser + 17 role constants + ref-key helpers.
│   └── schemas/         # 17 role-graph schema builders + schema_for_role dispatch.
