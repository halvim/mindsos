...
This page lists the decisions that are still **open**: ADRs whose status is Proposed or Deferred, plus open questions that never got an ADR. `tests/test_adr_status_consistency.py` checks every status this page states — a table cell, or a heading that names an ADR outside the record section — against the ADR file, so an ADR that ships and flips status turns that test red until it moves below.

Decisions made since this page was first written are kept under **Resolved — record** at the bottom, for their rationale only. Each summary there is the proposal as written in April 2026, not the shipped API; the ADR file and its amendments are authoritative.
...
### Pipeline generation as a capacity — Deferred (ADR-0082)
...
### Transitive promotion of dependencies — Deferred (ADR-0083)
...
### Cross-layer rewrite handler contract — ADR-0120

Deferred on its own file. It is the one ADR of the April 2026 Server-layer pivot that was written but not accepted; the pivot's record is below.
...
#### Property bag on `Metagraph` / `Graph` (ADR-0130)
...
The pivot planned eight ADR numbers. Four were written — 0114, 0115 and 0118 are Accepted, 0120 is Deferred; 0113, 0116, 0117 and 0119 are numbers not in use (see the [ADR index](adr/README.md)).

| ADR # | Title | Status | Supersedes |
|-------|-------|--------|------------|
| 0113 | Mutation model — Resolution A (auto-bump on mutation) | never written — number not in use | — |
| [0114](adr/0114-release-manifest-and-version-db-schema.md) | Release manifest + version DB schema (SQLite) | Accepted | parts of 0007 |
| [0115](adr/0115-release-ship-audit-gate.md) | Audit gate + impact report format | Accepted | 0009; parts of 0007 |
| 0116 | Edge soft-delete model (deprecated/disputed) | never written — number not in use | — |
| 0117 | Compositional metaedge immutability (`CompositionalMetaEdge`) | never written — number not in use | — |
| [0118](adr/0118-per-user-transactional-promotion.md) | Per-user transactional promotion + release-boundary atomicity | Accepted | 0007 in full |
| 0119 | Composition-signature dedup at promote | never written — number not in use | — |
| [0120](adr/0120-cross-layer-rewrite-handler-contract.md) | Cross-layer rewrite handler contract (KL + Capacity in v1) | Deferred | — |

#### Cross-layer impact carried by the pivot

Seven cross-layer decisions resolved in PIVOT §6.B, as recorded on 2026-04-26:
