...
## v2 closed role-set (post-Phase-43)

12 named role-graphs + an alignment-prefix family (`alignment:<a>:<b>`). Each carries an `L2Schema(Schema)` instance declaring its `mutation_discipline` per ADR-0153 §1.
...
The Phase 13 sentinel test `tests/phase_13/test_dispatch.py` enforces closure (12-entry exact-set check on `_ROLE_SCHEMA_BUILDERS`).
