...
- **L0-26 — ADR on disk; impl SHIPPED at Phase 50** (M3 → SA-1).
  ...
  Coverage: `tests/phase_50/test_adr_0182_value_codec.py` (the M3 sentinel was
  deleted and replaced by round-trip coverage — see §7 above).
...
| L0-25 | **FalkorDBLocalPersister live-FalkorDB integration test + scoped-delete coverage.** Phase 44 unit tests use `InMemoryClient` (no real round-trip); the save→load round-trip + the scoped `metagraph_id`-keyed delete Cypher are unvalidated against a live FalkorDB. Also: the delete's metaedge/metahyperedge/XRef sweep is a best-effort first cut needing completeness verification. Per `PHASE_44_DESIGN_LOG.md §7`. | Phase 44 (PR1.2) | ~~MAINTENANCE_CHAT~~ CLOSED 2026-06-09 (M2); ~~sweep audit~~ **AUDIT CLOSED Phase 51 (2026-06-10)** — sweep complete for owner-scoped rows (per-kind grounding in `PHASE_51_DESIGN_LOG.md`); orphan-scan xfail removed → contract; inbound XRefs pinned as by-design survivors (→ L0-27) |
| L0-27 | **Stamp `target_stale` on inbound XRefs at referent-delete.** `FalkorDBLocalPersister.delete` correctly leaves OTHER metagraphs' XRef rows that target the deleted Local (they are not owner-scoped; sweeping them would mutate a foreign metagraph outside its mutex + bypass its WAL). The ADR-0135 model already anticipates dangling targets (`build_set_xref_target_stale`); a referent-delete hook stamping `target_stale = true` on inbound rows — WAL-disciplined, per-source-metagraph mutex — would close the loop. Pinned by `tests/maintenance/test_l0_25_falkor_local_persister_live.py::test_live_delete_spares_inbound_xrefs_by_design`. | Phase 51 L0-25 audit | **Trigger: first cross-user XRef consumer** (none at v1 — `read_other_local` reads live, not via XRef) |
...
*End of L0_FUTURE_WORK.md. §7 closure markers added 2026-06-09 by MAINTENANCE_CHAT; staleness pass 2026-06-11 (dead M3-sentinel path → `tests/phase_50/test_adr_0182_value_codec.py`).*
