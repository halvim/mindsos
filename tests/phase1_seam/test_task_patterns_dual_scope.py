"""S0.5 — ``task-patterns`` is dual-scope (Global + Local), ADR-0150 §am-8.

Precursor to the Phase-1 interpretation seam (ADR-0195): a consumer's
``map`` body returns a ``task-pattern:*`` IRI authored in its Local scope.
This requires a Local ``task-patterns`` role-graph, which §am-8 adds by
...
def test_task_patterns_is_dual_scope() -> None:
    """§am-8 — task-patterns appears in BOTH the Global and Local named sets."""
    assert ROLE_TASK_PATTERNS in _GLOBAL_NAMED_ROLES
    assert ROLE_TASK_PATTERNS in _LOCAL_NAMED_ROLES


def test_ensure_local_creates_task_patterns() -> None:
    """ensure_local_role_graph no longer rejects task-patterns."""
    mg = Metagraph(name="local_t")
    g = ensure_local_role_graph(mg, ROLE_TASK_PATTERNS)
    assert g.role == ROLE_TASK_PATTERNS
    assert g.schema is not None


def test_lazy_local_contains_task_patterns() -> None:
    """A lazily-minted Local auto-ensures a task-patterns graph."""
    kl = KnowledgeLayer.bootstrap()
    local = kl.local_metagraph("alice")
    roles = {g.role for g in local.graphs.values()}
    assert ROLE_TASK_PATTERNS in roles
