...
def _begin_uncommitted_wal_entry(client, mid: str, kind: str, payload: dict) -> str:
    """Phase 09 idiom — use WriteAheadLog.begin() to write an uncommitted entry.

    Matches the Phase 09 test_xref_wal_recovery.py pattern.
    """
    wal = WriteAheadLog(client, mid)
    op_id = str(uuid4())
    wal.begin(operation_id=op_id, kind=kind, payload=payload)
    return op_id
