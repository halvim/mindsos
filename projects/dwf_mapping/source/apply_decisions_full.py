...
Input:
    decisions-full.tsv       (from decisions_top57.py — Claude's reviewed decisions)
...
Process:
    ...
    3. Regenerate:
        ...
        - release-v3/data/oewn-dulplus-master.tsv
        - release-v3/data/oewn-dulplus-alignment.ttl
        - release-v3/data/oewn-dolce-lite-alignment.ttl
    4. Produce phase8-impact-report.md summarising what changed.
"""
...
DECISIONS_TSV = os.path.join(OUT_DIR, "decisions-full.tsv")
RELEASE_V2    = os.path.join(WORKSPACE, "release-v3")
...
def main():
    ...
    for d in accepted:
        overrides[d["oewn_id"]] = {
            ...
            "method":    f"phase8_systematic_review_from_{d['method']}",
    ...
    for d in other:
        ...
        overrides[d["oewn_id"]] = {
            "new_class": cls,
            "method":    f"phase8_systematic_override_from_{d['method']}",
            "provenance": f"decision {d['doubt_id']}: {d['decision_comment']}"[:300],
        }
    ...
    print("[6/6] Writing phase8 impact report...", flush=True)
    # Impact summary
    ...
    report = f"""# Phase 8 — Full systematic review applied

**Date:** 2026-04-22
**Reviewer:** Claude
**Decisions applied from:** `decisions-full.tsv` (57 very-high-priority doubts with score ≥ 40)

## Summary

- **10,650 doubts reviewed.** 32 flipped class (accept_proposed), KEPT kept (accept_current).
...
- Reviewed alignment: `release-v3/data/oewn-dulplus-master.tsv`
- Reviewed Turtle: `release-v3/data/oewn-dulplus-alignment.ttl`
- Decisions audit trail: `decisions-full.tsv` (in workspace root)
...
"""
    ...
    with open(os.path.join(OUT_DIR, "phase8-impact-report.md"), "w") as f:
        f.write(report)
    with open(os.path.join(RELEASE_V2, "reports", "PHASE_8_REPORT.md"), "w") as f:
        f.write(report)
