...
- **Remaining m2 (next increment):** compose the minted *composite* = perception chain + `extract` + `matches`(stored definition) into a persisted, named, restart-surviving node (the teach-mint flow + reactivation factory carrying the definition in the descriptor, PB-9) + the human naming step + Linux integration gate.
