...
| **54** | `hint.*`/`predicate.*`/`decision.*` additive families (L4-consumed); atomic v0→real `planning.*`/`phase1.*`/`orchestration.*` flip (L4 orchestrator graduation); first-real-capacity-dispatch e2e milestone (closes Phase 49 PB-1a) | — | `wsd-pipeline` bundle; `usage/cookbook/nlu-slice.md` |
