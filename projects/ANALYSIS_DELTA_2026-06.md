...
| B20 (FOL pushback #9 — typed `CapacityContext`) | "L3 capacity-context redesign" (future) | **SHIPPED** Phase 42 (`context.py`: 10-field typed `CapacityContext` + 4 Protocols + 5 verdicts; 11th field `writeable` Phase 48 ADR-0180). Residual: read-path corpus migration + union-drop = **L3-59** (contract authority SKILL_ACQUISITION R0; mechanical migration WSD slot 1) | `mindsos_capacity/context.py`; `docs/future_work/L3_FUTURE_WORK.md` L3-59 |
