...
**Beat 4 — Cross-robot transfer + embodiment gate (headliner + the subtle bit).** The taught composite learned on Arm-1 is **peer-transferred Local→Local** to Arm-2 — **no Global write, no admin** (F1, DM-7 pick PB-1/PB-2): Arm-1 `bus.send`s the artifact; Arm-2's **receiver-side** handler writes it into **Arm-2's own Local** under Arm-2's own session via the ADR-0180 `writeable` gate. Arm-2 gains the knowledge **but the affordance gate still applies** (§5.2). The gate is evaluated **entirely against Arm-2's own embodiment** at dispatch — the transferred artifact carries no affordance memory of its origin — so Arm-2 (jaw) is refused `place-Sheet` (suction-only); Arm-1 (suction) is refused `place-Tube` (jaw-only). UI shows knowledge transferring while capability is body-gated. *(Probe-confirmed 2026-06-15: receiver-writes-own-Local + retire-safe `learned-parameters` + gate-on-receiver; design log §27.)*
...
1. **Pipeline artifact** (Seam D output → L5 Pipeline level → L2 promoted-pipelines) — **RESOLVED 2026-06-05: parameterized linear pipeline + explicit I/O bindings, scoped to within-brain composites.**
   ...
   - **DM-7 ships the Local-only path (pick PB-1, no Global promotion on stage):** `promoted-pipelines` is **Global-only** (schema), so a skill taught + not-yet-promoted has no Local node there. DM-7 represents the taught capability as a per-CL `install_override` body + a **Local `learned-parameters`** node (cache key/metadata) + a `TrajectoryCache` clip; transfer is the receiver-side Local↔Local path above. Global promotion (this lifecycle line) is the F5 forward-state, **not** exercised in DM-7. First-class Local `Pipeline` nodes would need a schema change → out of scope.
