...
| **C3R1b** | **[added 2026-08-01]** **Registration rejects a capacity whose outputs intersect its inputs.** A capacity that consumes and produces the same DataState IRI is a self-loop: it can never be the first producer of that DataState, and it is indistinguishable at the graph level from a legitimate producer, so every finder must carry a cycle guard to survive it. Under the model a *refined* DataState is a different DataState — `A` and `A'` are separated by the capacity that transforms one into the other, and that separation is what makes them distinct. The rule is therefore already implied by the concept; what is missing is that nothing enforces it. Lands in `capacity_layer.py::_validate_declaration`, beside the existing `input_group not in INPUT_GROUPS` check (`:452`), raising the same way. **Known violator: arc3's `grouped` verdict self-loop** — that chat must be told before this lands, and the sweep (C3R1) is how it finds any others. Rationale record: the check's own error message, ADR-0071 §am-4, and the `ConjunctionFinder` docstring, which currently documents a guard for a case that would no longer be representable | — |
