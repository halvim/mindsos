...
| **C3R1b** | **[added 2026-08-01]** **Registration rejects a capacity whose outputs intersect its inputs.** A capacity consuming and producing the same DataState IRI is a self-loop: it can never be the first producer of that DataState. Under the model a *refined* DataState is a different DataState — `A` and `A'` are separated by the capacity that transforms one into the other, and that separation is what makes them distinct. The rule is implied by the concept; nothing enforces it. Lands in `capacity_layer.py::_validate_declaration`. **Hygiene, NOT a fix for D-B** — measured over 20,000 catalogs, the self-loop ban halves the pre-stopgap failure rate (538 → 272) and does not close it, because the real cause is a cycle between two *distinct* capacities, which is legitimate and permanent (encode/decode, compress/decompress). **Known violator: arc3's `grouped` verdict self-loop** — that chat must be told before this lands. Rationale record: the check's own error message, ADR-0071 §am-4, and `CORE_CAPACITY_GRAPH_TRAVERSAL.md` §7.1 | — |
