...
> **HISTORICAL — read alongside `confirmation_docs/PHASE_05d_IMPLEMENTATION_LOG.md` §1.** This file documents rounds 1-6 (M1-M7 meta-plan + P1-P30 design picks) locked by the design chat 2026-05-07. The implementation chat ran a round-7 reanalysis pass (P31-P44) before any code landed; many picks below were reversed. Specifically: P31 A dropped the fingerprint mechanism + `--accept-vocab-change` flag + metagraph state-file bump (reverses P12 B / P14 A / P17 B / P26 B / P30 B / P20 A's metagraph bump). P32 A added `--schema MS` opt-in to `validate` (extends P18 B). P33 A struck P11 A. P39 A locked empty-vocab + non-strict eager-attach pass-silently. P40 A dropped `vocab_fingerprint_match` from `validate --json` (refines P25 A). P41 A split exit code 2 into 2/3 (refines P18 B). P42 C replaced P35/P36's would-have-been-inline ADR amendments with one-line pointers. P44 A factually corrected P5 A's "mirrors 05b" claim (the actual 05b order is different — verified at `metagraph.py:735-798`). For the canonical shipped state, read PHASE_MAP §5 Phase 05d row text + PHASE_05d_IMPLEMENTATION_LOG §1 (round-7 ledger), §3 (in-container hotfixes B-05d-T1 + B-05d-T2), §4 (P45 implementation pushback documenting the empty-vocab guard divergence from 05b/05c IntergraphEdge precedent — deliberate, not a bug).
