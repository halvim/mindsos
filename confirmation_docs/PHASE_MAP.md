...
### Phase 22 — Server: admin ops
  ...
  **Deps:** 19, 20, 21. **Layer:** L0. **Net-new?** No (extends `mindsos_server/admin.py` shipped at Phase 20).
  **Phase 20 PB-B inheritance:** Lands `_assert_not_sole_admin(conn, target_user_id)` helper + `LastAdminError(target_user_id)` class (deferred from Phase 20 — no P20 consumer; Phase 22 wires all three call sites in one chat). Reuses `UserNotFoundError` + `NotAnAdminError` from Phase 20 errors.py where target-validation parallels reset-admin's gate. Second-fire of `EVT_KILL_SESSION` (first-fired at Phase 20 by reset-admin) lands via `admin_kill_session`; second-fire of `EVT_ADMIN_ENABLE_USER` via `admin_enable_user`.
  **Features:** admin user mgmt (`admin_promote_user`, `admin_demote_user`, `admin_disable_user`, `admin_enable_user`); kill session (`admin_kill_session` gated by `CAN_KILL_SESSION`); cross-user read with refcount-install (ADR-0008); `hard_delete_user` (gated by `CAN_HARD_DELETE_ARCHIVED`). All three of demote/disable/hard-delete gated by `_assert_not_sole_admin` per ADR-0012 §am2 PB-B inheritance.
  **Tests:** non-admin call rejected; cross-user read leaves no flush behind; kill-session immediate; demote-disable-hard-delete on sole admin raises `LastAdminError`.
  **Risks:** admin actions cross the privacy boundary; audit must be exhaustive.
  **Docs:** `docs/usage/server/sessions.md`, ADR-0008, ADR-0012 (`_assert_not_sole_admin` first-construction at P22 per §am2 PB-B).
