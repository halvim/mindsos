...
| 24 | Server + admin: per-user transactional promotion (admin-direct ATOM only) + RELEASE_SHIP_LOCK + audit gate (two-pass `compute_similarity` per PB-24) + release manifest (`pending_mutations` + `releases` schema v3 → v4) + FAILED-row forensics. ADR-0129 §am2 drops snapshot from `release_update` (vestigial in halvim per PB-7 probe). Lazy migration + source-user-Local propose path + STRUCTURE/SUBGRAPH/PIPELINE validators all deferred to later phases. ADRs 0114 + 0115 drafted + Accepted; ADR-0120 drafted (Proposed; impl at P25). ADR-0118 + ADR-0141 → Accepted with §am1; ADR-0144 → fully Accepted (§am1 retires; §am2). ADR-0007 → Superseded. ADRs 0049/0053/0056 → Superseded (per Phase 16 §am1 lock). ADR-0002 §am2 (+2 caps: PROPOSE_MUTATION + APPROVE_RELEASE). ADR-0006 §am1 (RELEASE_SHIP_LOCK rename ratified). ADR-0129 §am2 (snapshot vestigial; module retained; CI lint rule dropped). **NEW CODE.** | L0/admin | 16 |
