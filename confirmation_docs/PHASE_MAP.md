...
### Phase 24 — Server: per-user transactional promotion (full ADR-0118 + ADR-0141 + ADR-0144 §Placement)
  ...
  **Deps:** 16. (Phase 23 dep dropped — Phase 23 retired 2026-05-22 per ADR-0129 §amendment-1.) **Layer:** L0 / L2 (admin). **Net-new?** **Yes** (absorbs `mindsos_admin/promotion.py` from Phase 16 PB-1c deferral + RELEASE_SHIP_LOCK from Phase 23 PB-4c absorption + STRUCTURE/SUBGRAPH/PIPELINE proposers + release manifest + lazy migration + Phase 23 retirement absorption below).
  **Phase 23 retirement absorption (2026-05-22 — ADR-0129 §amendment-1):** Phase 24 lands the snapshot rollback bracketing per ADR-0129 §amendment-1 §1: `release_update` calls `snap = MetagraphSnapshot.of(canonical_global_mg)` BEFORE the per-role `pending_global_<role>` → `mindsos_global_<role>` copy. On exception during any per-role copy, `release_update` calls `snap.restore_into(canonical_global_mg)` and re-raises. **Inline pattern only — no wrapper module, no contextmanager.** Audit-gate sequencing per ADR-0118 §"Decision" §2: snapshot is taken AFTER the pre-ship audit gate (ADR-0115) passes and AFTER `RELEASE_SHIP_LOCK` is acquired, BEFORE the first per-role copy. Pending stays intact for retry on rollback (no cross-user state is touched per ADR-0118 §"Per-user migration"). Phase 24 also ships the CI lint rule (`grep MetagraphSnapshot.of(` outside `mindsos_server/` in `tests/unit/test_layer_isolation.py` — or halvim's equivalent location; rescheduled here from the original Phase 10 design Q "Phase 18+" deferral that was silently missed across Phases 18-22). ADR-0007 flips to Superseded when this code lands per its supersession-in-progress banner.
