...
- **DECISION 2026-06-15 (DM-8 reanalysis): SCHEDULED as a dedicated MindsOS chat — "Full F9".** Driven by the DM-8 Mode-B reload pick (user chose reboot-survival over in-process import). **Grounding (probe):** part (a) is **~80% shipped** — `FalkorDBLocalPersister` (`mindsos_server/persistence/local_persister.py`, Phase 44: `load`/`save`/scoped `delete`, native round-trip, tested) + the Phase-50 `value_codec` (`mindsos_core/persistence/value_codec.py`, ADR-0182) that fixed dict-valued nodes (Episode / `learned-parameters`), wired into the graph repository/loader the persister routes through; D'1 retention markers are plain properties → survive round-trip. **But dormant:** not exported in `mindsos_server.persistence.__all__`, zero v1 consumer (Phase-44 CR-3 deferred login/logout Local writes). Part (b) is the **net-new** half: `CapacityLayer._declarations` is in-memory and a capability `implementation` is a live Python callable that **cannot be serialized** → re-activation must be **descriptor/factory-driven, not pickled** (`_CapacityBase.from_properties()` + a re-activation registry; descriptor-driven caps like learned skills + bundle skills rehydrate from their persisted descriptor; arbitrary code-bodied caps re-activate by re-running their installer). **Key reframe:** the demo's own learned skill rehydrates from its `learned-parameters` descriptor via a boot-time `teach_local` replay (consumer-side, no `mindsos_*` edit) — so Minimal F9 (part a only) would already unblock the demo; **Full F9 was chosen to also ship the generic re-activation contract.** Subtle design point: the demo deliberately keeps Locals in-memory (PB-Z) for a deterministic boot smoke + trivial reset → F9 must define the **persist/retain vs reset boundary** (learned skills survive; episodes/placed-orders reset) using the existing `FalkorDBLocalPersister.delete`. **Sequencing:** Full F9 MindsOS chat (ADRs + phase + Linux gate) → then DM-8 consumes it (`load_or_mint_local` + boot re-teach + `import_state`/Mode-B). Full handoff: `confirmation_docs/F9_DURABLE_LOCAL_PERSISTENCE_NEXT_CHAT_PROMPT.md`.
