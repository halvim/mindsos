...
| Phase | Scope | Gate |
|---|---|---|
| **DM-1** ✅ | §1 deployment: compose service, `demo_backend` skeleton, bootstrap (admin, 4 users, Global bootstrap, smoke lifecycle ×4), RAM measurement | **DONE — green on Linux 2026-06-11** (4 brains, 4 Episodes, idempotent re-boot; RAM 38.3 MB, jitter p99 20.08 ms; PB-V session-clear fix). See DESIGN_LOG §6/§9. Code: `robot_demo/` |
| **DM-2** ✅ | §4.0 DataStates + skill bundles (§3.4) + Local seeds (§3.3) + G-5 episode flush probe | **DONE — green on Linux 2026-06-11.** 5 bundles (`core`→all 4; `manager`/`arm-suction`/`arm-jaw`/`conveyor`→their device) install idempotently through the real `install_skill` gate (no-op on reload); Local embodiment seeds visible; G-5 dict-valued Episode round-trips the ADR-0182 codec. Build-forced changes (DESIGN_LOG §10–§14): schema enforces NodeType registration even at `strict=False` + bundles seed nodes-only → all requirements/affordances property-encoded (PB-W/PB-DD); Globals persisted, Locals in-memory (PB-Z); demo data isolated in the `robot_demo` graph (PB-JJ); F8/F9 filed. Code: `robot_demo/` |
| **DM-3** ✅ | Seam C: body adapter + atomic capacities + live-motion wrapper (G-7); fault switch (G-8) | each atomic capacity moves the live sim, checklist-verified — **GATE GREEN ON LINUX 2026-06-11.** Single shared SimEngine (PB-KK) + per-brain BodyHandle; 10 atomic ⬡ capacities (move_to/grip/sense/diagnose; conv.run/stop); subset `atomic_checklist` = jerk + conveyor hard gate (PB-LL, §18 calibration); cache+live-gen+dont-know (PB-F); fault clamp + Local gap (PB-NN/K); MuJoCo behind the seam (PB-TT). Real jitter (PB-RR): baseline p99 24.9 ms / under-4-workers 22.2 ms vs 20 ms nominal. 5 gate-iteration fixes (vendored menagerie+reach_validate committed, mujoco→3.3.0, PEP668, move_belt slice, checklist+target calibration). Zero `mindsos_*` edits / zero edits to existing `sim/` source. Design log §15–§18 |
| **DM-4** ✅ (core) | Seam B: BrainBus + `comms.*` (push-cache, PB-C); G-1/PB-B resolution; **thin vertical UI slice (PB-G): live WS state/message/pose frames the UI seam consumes** | **CORE GATE GREEN ON LINUX 2026-06-12.** mgr dispatches a1 **`move_to`** (gate text's `place_at_cell` is ◆ DM-5 — reconciled to a shipped ⬡ atomic) end-to-end through both lifecycles over the BrainBus, narrated as live IP-sanitized frames; `body=yes`, real motion + 27 pose frames. Probed reversals (DESIGN_LOG §19): `if_exists="upsert"` is a behavioural no-op → override via `cl._declarations` swap (PB-WW); params via `task_pattern_iri` (PB-EEE); dedicated bus consumer thread (PB-FFF); pose affine fitted top-down `(x,y)`, `bodies` dropped. PB-HHH per-task Orchestrator scope (2nd-lifecycle `IdentityError`) + reload DataState re-register + sanitization guard. Zero `mindsos_*` edits. **Remaining in DM-4:** Mode-A L5 export + `server_status` (coordination doc locked); browser confirmation. Design log §19–§21 |
