...
| **DM-3** ◑ | Seam C: body adapter + atomic capacities + live-motion wrapper (G-7); fault switch (G-8) | each atomic capacity moves the live sim, checklist-verified — **code landed 2026-06-11 (Cowork + sandbox: 23 pass/6 skip); Linux gate pending Mac commit + run.** Single shared SimEngine (PB-KK) + per-brain BodyHandle; 10 atomic ⬡ capacities (move_to/grip/sense/diagnose; conv.run/stop); subset `atomic_checklist` (PB-LL); cache+live-gen+dont-know (PB-F); fault clamp + Local gap (PB-NN/K); MuJoCo behind the seam (PB-TT). Zero `mindsos_*`/`sim/` edits. Design log §15-§17 |
