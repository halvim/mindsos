...
| **Manifest `[mindsos] phase` field (high-water mark — amended Phase 40)** | The manifest `[mindsos] phase` + all version surfaces track the **highest confirmed phase**, not the slot currently shipping. Under the DAG, phases ship out of numeric order (Phase 44 shipped before Phase 40), so a rail slot numerically ≤ the current high-water mark ships **without** a version bump — its release artifact *is* the high-water version; the `phase-NN-confirmed` tag distinguishes it. `mindsos confirm-phase --phase NN` accepts any slot ≤ the manifest phase and rejects only a slot strictly ahead (`confirm_phase._phase_exceeds_manifest`, shipped Phase 40); the CONFIRMED.md `image_tag` derives from the manifest phase. The original PB-S note ("phases stay monotonic, no collision arises") missed this out-of-order-confirm case. `doctor --self-test` unchanged — compose tags + the 7 `__version__` + pyproject stay internally consistent at the high-water phase. **Slots that bump:** only a phase numerically > the current high-water mark (e.g. 46 over 44) runs the 9-surface bump; 40/41/42 (≤44) do not. |
...
| **6-step confirm-phase workflow (Phase 43 R12 carry-forward)** | Established pattern for the `mindsos confirm-phase` cycle, using the two canonical CLI commands: **(a)** `mindsos confirm-phase --init-notes N` generates `confirmation_docs/notes/notes-phase-N.md` from template; **(b)** `mindsos confirm-phase --phase N --notes-file notes-phase-N.md` writes `PHASE_N_CONFIRMED.md`. The 6 steps: (1) Cowork instructs `mindsos confirm-phase --init-notes N` (Mac) to create the notes file from template; (2) Cowork provides the layer title in a copy-block; (3) Cowork provides the complete `tester_notes` body in a copy-block (drawn from cumulative gate output + design log §9 content); (4) tester edits the notes file on Linux; (5) tester runs `mindsos confirm-phase --phase N --notes-file notes-phase-N.md` on Linux from post-squash main; (6) tester commits `PHASE_N_CONFIRMED.md` + notes-phase-N.md + pushes. See `PHASE_43_DESIGN_LOG.md §9.1` R12. |
...
### Phase 42 — L3 X3: bipartite topology + capacity registration contract v2 + Phase 27 audit + Model C remediation
  ...
  **Locked decisions:**
  ...
  - **PB-8 (from Phase 40 ship — see PHASE_40_DESIGN_LOG §8 + HANDOFF §3.1.15):** the Phase 27 audit MUST also reconcile the shipped `FAMILY_RULES` key-vocabulary against the actual `FUNCTIONAL_CATEGORIES`. Phase 40 transcribed ADR-0157's dict verbatim; its keys use `derive`/`signal` (shipped categories are `derivation`/`signalling`) and omit comprehension/decomposition/path_finding/trace/interaction/learning_methods/consolidate → those 9 shipped categories resolve via the permissive `DATASTATE_MARKER` default. Latent at v1 (all 3 shipped capacities classify correctly). The audit decides: amend ADR-0157's dict (rename keys + add the 7 categories with chosen shapes) or keep the default. Tracked as L3-57.
...
### Phase 44 — L0 substrate: persisters + KL surface + bootstrap completion + audit
  ...
  **R0 reconciliation (2026-06-04, Phase 44 chat):**
  ...
  - **CR-2 ruling (revised 2026-06-04): Falkor-only v1.** Initially "ship both"; reversed on PR1.2 investigation — the `mindsos_cli` state-file serializer is disk-coupled/multi-file (reuse = real refactor + CLI-regression risk) and `SQLiteLocalPersister` has no v1 consumer. Ship `FalkorDBLocalPersister` native (`MetagraphRepository.persist` / `MetagraphLoader.load`, no serialization). **Deferred:** `SQLiteLocalPersister` + `MetagraphDump` + `locals.db` + the `mindsos_cli`→`mindsos_core` serializer promotion → first local-first/portable-export consumer. Drops `tests/phase_44/test_sqlite_persister.py` from this phase. See design log §6.
  - **CR-3 ruling (revised 2026-06-04): defer the `MindsOSServer` class refactor.** Initially "do it now"; reversed on PR2 grounding — the class's justifications eroded (SQLite deferred + login/logout hydrate/flush hooks have no v1 consumer; nothing writes user Locals until L4/L5). Orchestrator stays free-function per PB-38; class + hooks land at the L4/L5 Local-write phase. PR2 dropped; phase = PR1 + PR3. See design log §9.
...
| **SKILL_ACQUISITION_PROCESS_CHAT** | Phase 47 confirmed (substrate must exist before skill-install lifecycle saturates) | WSD installation R0 | Per-layer install lifecycle (skill = L1+L2+L3+L4+L5 artifacts as bundle); bundle integrity; Local vs Global tiers; conflict resolution; audit + provenance; de-installation | Per `projects/README.md` ordering + PB-H. Shared umbrella for WSD + FOL installations. **CLOSED 2026-06-09** — contract at `SKILL_ACQUISITION_PROCESS_DESIGN_LOG.md`; sequencing at `SKILL_ACQUISITION_PROCESS_PHASE_MAP.md` (SA-1 = Phase 50, ships the install driver + ADR-0182 impl + `installed-skills` role-graph). |
| **WSD_INSTALLATION_CHAT** | SKILL_ACQUISITION_PROCESS_CHAT closure | First WSD `<phase>-confirmed` tag | WSD skill installation phase-map; absorbs L0 admin-surface items per PB-T (audit constants for ALS events, scheduler infra, HITL channel, capacity-gaps tooling, hint catalog tooling); replaces Phase 47 `planning.*` v0 with real catalog; ships ALS subsystems #1-#11 mechanism + validator catalogs; ships `process.*`, `predicate.*`, `hint.*`, `decision.*` catalogs; ships `world-axioms` role-graph; ships 6 new L2 importers (SemCor, OntoNotes, VerbNet, SemLink, GlossTag, FrameNet-extended); ratifies WSD-specific ADRs from `projects/wsd/source/pending_adrs/`; **ships the promotion-loop mechanism** (writers/consumers of `parameter-staging` → `pending-promotions` → `learned-parameters`) under the SKILL_ACQUISITION producer-agnostic contract (design log S10; routing amendment 2026-06-09 — closes the loop's ownership gap) | Per `projects/wsd/FUTURE_CHAT_PROMPT.md`. Scope is large — likely multi-chat sub-series. **Design CLOSED 2026-06-10** — sequencing at `WSD_INSTALLATION_PHASE_MAP.md` (Phases 51–56 reserved); **this row's roster was scope-amended at closure** (importers 6→2 SemCor+GlossTag; ALS = consumed subsystems only; `world-axioms` deferred to ledger; PB-T trimmed) — phase-map §4 is authoritative over this row. **Ownership note (2026-06-25): this row lists items as WSD scope, but most are MindsOS-core mechanics WSD merely consumes (promotion-loop, `planning.*` v0→real flip, `process/predicate/hint/decision` catalogs, ALS contract, empirical-layer machinery). WSD is scheduled to build them, not their owner — authoritative per-item ownership = `projects/wsd/WSD_INSTALLATION_PHASE_MAP.md` §2.1 Owner column + design log §7.** **EXTRACTION (2026-07-31): the `MindsOS-core` items in this row are no longer WSD-scheduled — they are re-sequenced into `confirmation_docs/CORE_RECONCILIATION_PLAN.md` and WSD *consumes* them. The 2026-06-25 pass fixed attribution; this fixes scheduling, so core no longer waits on a subsystem (RULES §8, ADR-0205).** |
...
| q4 | `planning.*` v0 → real catalog migration discipline | **CLOSED 2026-07-31 by ADR-0206** (planning as a loop; confidence stopping rule; placeholders deleted, reference bundle becomes the fixture). Build sequenced as CORE-C4R4/C4R8. |
