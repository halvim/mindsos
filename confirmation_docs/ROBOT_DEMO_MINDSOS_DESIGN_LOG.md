...
Files changed across the iteration: `sim/reach_validate.py` + `sim/menagerie/` (vendored), `robot_demo/requirements-demo.in`, `robot_demo/deploy/run_linux_tests.sh`, `robot_demo/backend/{sim_engine,motion_checklist,dm3_check}.py`. Still **zero `mindsos_*` edits / zero edits to existing `sim/` source**. Awaiting the final Linux gate run to confirm the in-container result matches the sandbox.
