...
- **2 — nesting.** A map's sub-plan may itself contain a map/fold (objects within grids). Mainly: per-member run-ref becomes a path so grounding stays isolated and the provenance tree is readable.
- **3 — per-member replan/diagnosis.** Point replan/diagnosis at a specific member using the reserved `"map"` / `"plan_subtree"` levels (`replan_check.py:19`) instead of whole-pipeline.
