...
from . import grids as arc_grids
...
def _touching_delta(**kw: Any) -> dict:
    ...
    from . import solver as arc_solver
...
_CAP_GROUPS = (_perceive_capacities, _profiler_capacities, _comparator_capacities,
               _intra_grid_capacities, _operator_capacities, _transform_capacities,
               _reason_capacities, _solver_capacities)


def _all_capacities() -> List[Capacity]:
    return [cap for group in _CAP_GROUPS for cap in group()]


def _global_presence(capacity_layer: CapacityLayer, dss, caps):
    """(datastates_present, capacities_present) counts on the Global metagraph."""
    mg = capacity_layer.global_metagraph()
    ds_graph = ensure_datastate_graph(mg, strict=capacity_layer._strict)
    cap_index = capacity_layer._capacity_index.get(mg.metagraph_id, {})
    ds_present = sum(1 for d in dss if d.iri in ds_graph.nodes)
    cap_present = sum(1 for c in caps if c.iri in cap_index)
    return ds_present, cap_present
...
def install_arc(capacity_layer: CapacityLayer, session: Any = None) -> None:
    ...
    install-path / demo gate). ``session`` present → the user's **Local** L3
    (DataStates AND caps together — they must share scope; a Local cap referencing
    a Global DataState raises). Verified: all-Local registration round-trips.

    Warm-layer idempotent on the Global path (builtins-triple, matching the ref
    installer): all-present → no-op; partial → raise; none → install. This is
    required by the ADR-0183 activation contract (``apply_installed_skills`` re-runs
    installers on every boot; on a warm layer they must no-op). The ``session`` (Local)
    path is only reached by the in-process harness, which always builds a fresh layer,
    so the guard is scoped to the Global metagraph."""
    dss = arc_datastates()
    caps = _all_capacities()
    if session is None:
        ds_present, cap_present = _global_presence(capacity_layer, dss, caps)
        present, total = ds_present + cap_present, len(dss) + len(caps)
        if present == total:
            return  # warm layer — no-op
        if present != 0:
            raise CapacityRegistrationError(
                "install_arc: partial ARC install state detected "
                f"(datastates {ds_present}/{len(dss)}, "
                f"capacities {cap_present}/{len(caps)})"
            )
    for ds in dss:
